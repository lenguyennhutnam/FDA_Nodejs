...
| Web               | Next.js 16 App Router (Proxy Refresh Token)  |
| Cơ sở dữ liệu | SQLite (better-sqlite3) qua TypeORM               |
| DB File           | `apps/api/data/fda.db`                          |
...
- **Node.js** ≥ 20
- **Cơ sở dữ liệu**: Chạy hoàn toàn trên **SQLite** cục bộ (tự động tạo tại `apps/api/data/fda.db`). *Lưu ý: Biến `MONGODB_URI` trong `.env` vẫn cần khai báo để vượt qua bước kiểm tra cấu hình (validation), nhưng hệ thống không kết nối tới MongoDB.*
...
*(Hoặc chạy `npm run seedAdmin` nếu sử dụng npm)*

Tài khoản tạo ra sẽ lấy theo cấu hình ở `apps/api/.env` (mặc định: `admin@fda.local` / `Admin@12345`). Hãy đổi mật khẩu sau khi đăng nhập lần đầu.
